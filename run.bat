...
if exist "venv\Scripts\activate.bat" (
    ...
    echo Recomendamos executar setup_complete.bat primeiro.
    echo.
)
...
if errorlevel 1 (
    echo ❌ ERRO: Dependências faltando! Execute setup_complete.bat
    pause
    exit /b 1
)
...
echo 🤖 IA: Google Gemini Pro + HuggingFace + Groq
echo 🔍 Pesquisa: Google Search + Bing + DuckDuckGo
...
echo.
echo 💡 DICAS:
echo - Para funcionalidades assíncronas: start_celery.bat
echo - Para monitoramento: start_flower.bat
...
echo 🔧 Para reconfigurar, execute setup_complete.bat
echo.
pause